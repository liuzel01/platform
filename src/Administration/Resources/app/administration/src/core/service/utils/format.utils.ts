...
export function currency(
    ...
    const opts = {
        style: 'currency',
        currency: sign || Shuwei.Context.app.systemCurrencyISOCode as string,
        ...decimalOpts,
        ...additionalOptions,
    };

    // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access,@typescript-eslint/no-unsafe-argument
    return val.toLocaleString((additionalOptions.language ?? Shuwei.State.get('session').currentLocale) ?? 'en-US', opts);
}
...
export function date(val: string, options: DateFilterOptions = {}): string {
    ...
    const lastKnownLang = Shuwei.Application.getContainer('factory').locale.getLastKnownLocale();
    // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
    const userTimeZone = (Shuwei?.State?.get('session')?.currentUser?.timeZone) ?? 'UTC';
