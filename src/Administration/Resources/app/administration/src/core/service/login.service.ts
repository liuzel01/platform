...
export default function createLoginService(
    ...
    function verifyUserToken(password: string): Promise<string> {
        ...
        return verifyUserByUsername(Shuwei.State.get('session').currentUser.username, password)
            .then(({ access }) => {
                if (Shuwei.Utils.types.isString(access)) {
                    return access;
                }
    ...
    function shouldConsiderUserActivity(): boolean {
        const devEnv = Shuwei.Context.app.environment === 'development';
    ...
    function forwardLogout(isInactivityLogout: boolean, shouldRedirect: boolean): void {
        ...
        // @ts-expect-error
        const $router = Shuwei.Application.getApplicationRoot().$router as null | VueRouter;
        if ($router) {
            const id = Shuwei.Utils.createId();
            ...
            if (isInactivityLogout && shouldRedirect) {
                ...
                void html2canvas(document.querySelector('#app'), { scale: 0.1 }).then(canvas => {
                    ...
                    window.sessionStorage.setItem('lastKnownUser', Shuwei.State.get('session').currentUser.username);
