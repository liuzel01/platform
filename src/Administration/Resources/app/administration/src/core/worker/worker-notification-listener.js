...
const { Application, WorkerNotification } = Shuwei;
...
class WorkerNotificationListener {
    ...
    start() {
        if (!Shuwei.Context.app.config.adminWorker.enableQueueStatsWorker) {
            return;
        }
    ...
    _checkQueue() {
        ...
        const client = Shuwei.Application.getContainer('init').httpClient;
        const headers = {
            headers: {
                Authorization: `Bearer ${Shuwei.Service('loginService').getToken()}`,
            },
        };
        ...
        client.get('/_info/queue.json', headers).then((res) => {
            ...
            if (this._isRunning) {
                this._interval = Shuwei.State.get('notification').workerProcessPollInterval;
                this._timeoutId = setTimeout(this._checkQueue.bind(this), this._interval);
            }
        });
    }
