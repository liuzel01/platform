...
export default function createRouter(Router, View, moduleFactory, LoginService) {
    ...
    function beforeRouterInterceptor(router) {
        ...
        router.beforeEach((to, from, next) => {
            const cookieStorage = Shuwei.Service('loginService').getStorage();
            ...
            const tokenHandler = new Shuwei.Helper.RefreshTokenHelper();
            ...
            if (to.meta.privilege && !Shuwei.Service('acl').can(to.meta.privilege)) {
    ...
    function convertRouteComponentToViewComponent(route) {
        const hasOwnProperty = Shuwei.Utils.object.hasOwnProperty;
    ...
    function getViewComponent(componentName) {
        if (vue3) {
            return Shuwei.Application.view.getComponentForRoute(componentName);
        }

        return Shuwei.Application.view.getComponent(componentName);
    }

    function getAssetPath() {
        return Shuwei.Context.api.assetsPath;
    }
