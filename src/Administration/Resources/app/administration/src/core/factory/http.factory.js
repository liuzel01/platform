...
function createClient() {
    const client = Axios.create({
        baseURL: Shuwei.Context.api.apiPath,
    });
...
function globalErrorHandlingInterceptor(client) {
    client.interceptors.response.use(response => response, error => {
        ...
        try {
            ...
            Shuwei.Utils.debug.error(e);

            if (errors) {
                errors.forEach(singleError => {
                    Shuwei.State.dispatch('notification/createNotification', {
                        variant: 'error',
...
/**
 * Determines the different status codes and creates a matching error via Shuwei.State
 * @param {Number} status
 * @param {Array} errors
 * @param {Object} error
 * @param {Object} data
...
function handleErrorStates({ status, errors, error = null, data }) {
    ...
    const viewRoot = Shuwei.Application.view.root;
        ...
        if (status === 401) {
            ...
            return new Promise((resolve, reject) => {
                tokenHandler.subscribe(
                    ...
                    (err) => {
                        if (!Shuwei.Application.getApplicationRoot()) {
                            reject(err);
