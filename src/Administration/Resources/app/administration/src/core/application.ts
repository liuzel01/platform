...
class ApplicationBootstrapper {
     ...
     *
     * @example
     * Shuwei.Application.addFactory('module', (container) => {
     ...
     *
     * @example
     * Shuwei.Application.addFactoryMiddleware((container, next) => {
     *    // Do something with the container
     *    next();
     * });
     *
     * @example
     * Shuwei.Application.addFactoryMiddleware('module', (service, next) => {
     ...
     *
     * @example
     * Shuwei.Application.addFactoryDecorator((container, next) => {
     *    // Do something with the container
     *    next();
     * });
     *
     * @example
     * Shuwei.Application.addFactoryDecorator('module', (service, next) => {
     *    // Do something with the service
     *    next();
     * });
     */
     ...
     *
     * @example
     * Shuwei.Application.addInitializer('httpClient', (container) => {
     ...
     *
     * @example
     * Shuwei.Application.addServiceProvider('productService', (container) => {
    ...
    registerApiContext(context: ContextState['api']): ApplicationBootstrapper {
        Shuwei.Context.api = Shuwei.Classes._private.ApiContextFactory(context);
    ...
    registerAppContext(context: ContextState['app']): ApplicationBootstrapper {
        Shuwei.Context.app = Shuwei.Classes._private.AppContextFactory(context);
     ...
     *
     * @example
     * Shuwei.Application.addServiceProviderMiddleware((container, next) => {
     *    // Do something with the container
     *    next();
     * });
     *
     * @example
     * Shuwei.Application.addServiceProviderMiddleware('productService', (service, next) => {
     *    // Do something with the service
     *    next();
     * });
     */
    ...
    initializeFeatureFlags(): ApplicationBootstrapper {
        const features = Shuwei.Context.app.features;
     ...
     *
     * @example
     * Shuwei.Application.addServiceProviderDecorator((container, next) => {
     *    // Do something with the container
     *    next();
     * });
     *
     * @example
     * Shuwei.Application.addServiceProviderDecorator('productService', (service, next) => {
     *    // Do something with the service
     *    next();
     * });
     */
    ...
    bootFullApplication(): Promise<void | ApplicationBootstrapper> {
        ...
        return this.initializeInitializers(initContainer)
            ...
            .then(() => Promise.all(Shuwei.Plugin.getBootPromises()))
            .then(() => {
    ...
    createApplicationRoot(): Promise<ApplicationBootstrapper> {
        ...
        if (Shuwei.Context.app.environment === 'testing') {
        ...
        const firstRunWizard = Shuwei.Context.app.firstRunWizard;
        // eslint-disable-next-line @typescript-eslint/no-unsafe-call,@typescript-eslint/no-unsafe-member-access
    ...
    private async loadPlugins():Promise<(unknown[] | null)[]> {
        ...
        if (isDevelopmentMode === 'development') {
            ...
            // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
            plugins = Shuwei.Context.app.config.bundles as bundlesPluginResponse;
        }
        ...
        // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
        const bundles = Shuwei.Context.app.config.bundles as bundlesPluginResponse;
        Object.entries(bundles).forEach(([bundleName, bundle]) => {
            ...
            if (isDevelopmentMode === 'development') {
                ...
                Object.entries(plugins).forEach(([pluginName, entryFiles]) => {
                    const stringUtils = Shuwei.Utils.string;
    ...
    private injectIframe({
        ...
        const bundles = Shuwei.Context.app.config.bundles;
        ...
        Shuwei.State.commit('extensions/addExtension', extension);
    }
}
