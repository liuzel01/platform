...
import type { Entity } from '@58shuwei-ag/admin-extension-sdk/es/data/_internals/Entity';
...
export default class Repository<EntityName extends keyof EntitySchema.Entities> {
    ...
    get schema(): EntityDefinition<any> {
        return Shuwei.EntityDefinition.get(this.entityName);
    }

    /**
     * Sends a search request to the server to find entity ids for the provided criteria.
     */
    searchIds(criteria: Criteria, context = Shuwei.Context.api): Promise<IdSearchResult> {
     ...
     */
    search(criteria: Criteria, context = Shuwei.Context.api): Promise<EntityCollection<EntityName>> {
        const headers = this.buildHeaders(context);
     ...
     */
    get(id: string, context = Shuwei.Context.api, criteria: Criteria | null = null): Promise<Entity<EntityName> | null> {
     ...
     */
    save(entity: Entity<EntityName>, context = Shuwei.Context.api): Promise<void | AxiosResponse> {
    ...
    clone(entityId: string, context = Shuwei.Context.api, behavior: $TSDangerUnknownObject): Promise<unknown> {
        if (!entityId) {
     ...
     */
    saveAll(entities: EntityCollection<EntityName>, context = Shuwei.Context.api): Promise<unknown> {
     ...
     * @private
     */
    sendUpserts(changeset: $TSDangerUnknownObject[], failOnError: boolean, context = Shuwei.Context.api): Promise<void> {
     ...
     */
    assign(id: string, context = Shuwei.Context.api): Promise<AxiosResponse> {
        const headers = this.buildHeaders(context);
     ...
     */
    delete(id: string, context = Shuwei.Context.api): Promise<AxiosResponse> {
        const headers = this.buildHeaders(context);
     ...
     */
    syncDeleted(ids: string[], context = Shuwei.Context.api): Promise<void> {
        const headers = this.buildHeaders(context);
     ...
     */
    create(context = Shuwei.Context.api, id: string | null = null): Entity<EntityName> {
     ...
     */
    mergeVersion(versionId: string, context = Shuwei.Context.api): Promise<AxiosResponse> {
        const headers = this.buildHeaders(context);
     ...
     */
    deleteVersion(entityId: string, versionId: string, context = Shuwei.Context.api): Promise<AxiosResponse> {
        const headers = this.buildHeaders(context);
     ...
     */
    buildHeaders(context = Shuwei.Context.api): {
        Accept: string,
        ...
        const { hasOwnProperty } = Shuwei.Utils.object;
