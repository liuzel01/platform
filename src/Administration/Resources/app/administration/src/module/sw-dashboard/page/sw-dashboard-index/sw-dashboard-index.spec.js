...
async function createWrapper(privileges = []) {
    return shallowMount(await Shuwei.Component.build('sw-dashboard-index'), {
        stubs: {
            'sw-page': true,
            'sw-card-view': true,
...
describe('module/sw-dashboard/page/sw-dashboard-index', () => {
    ...
    beforeAll(async () => {
        if (Shuwei.State.get('session')) {
            Shuwei.State.unregisterModule('session');
        }

        Shuwei.State.registerModule('session', {
            state: {
                currentUser: null,
            },
            mutations: {
