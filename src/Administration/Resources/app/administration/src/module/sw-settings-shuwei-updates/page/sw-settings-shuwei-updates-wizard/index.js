import template from './sw-settings-shuwei-updates-wizard.html.twig';
import './sw-settings-shuwei-updates-wizard.scss';
...
Component.register('sw-settings-shuwei-updates-wizard', {
    template,
    ...
    computed: {
        ...
        updateButtonTooltip() {
            ...
            return {
                message: this.$tc('sw-settings-shuwei-updates.infos.requirementsNotMet'),
                position: 'bottom',
            };
        },
        ...
        optionDeactivateIncompatibleTranslation() {
            const deactivateIncompatTrans = this.$tc('sw-settings-shuwei-updates.plugins.actions.deactivateIncompatible');
            const isRecommended = this.displayIncompatiblePluginsWarning && !this.displayUnknownPluginsWarning ?
                this.$tc('sw-settings-shuwei-updates.plugins.actions.recommended') : '';

            return `${deactivateIncompatTrans} ${isRecommended}`;
        },

        optionDeactivateAllTranslation() {
            const deactiveAllTrans = this.$tc('sw-settings-shuwei-updates.plugins.actions.deactivateAll');
            const isRecommended = this.displayIncompatiblePluginsWarning && this.displayUnknownPluginsWarning ?
                this.$tc('sw-settings-shuwei-updates.plugins.actions.recommended') : '';
    ...
    methods: {
        ...
        startUpdateProcess() {
            ...
            this.createNotificationSuccess({
                message: this.$tc('sw-settings-shuwei-updates.notifications.updateStarted'),
            });
        ...
        stopUpdateProcess() {
            ...
            this.createNotificationInfo({
                message: this.$tc('sw-settings-shuwei-updates.notifications.updateStopped'),
            });
        },
        ...
        downloadRecovery(offset) {
            this.updateService.downloadRecovery(offset).then(() => {
                ...
                this.createNotificationError({
                    message: this.$tc('sw-settings-shuwei-updates.notifications.downloadFailed'),
                });
            });
        },
        ...
        deactivatePlugins(offset) {
            ...
            this.updateService.deactivatePlugins(offset, this.chosenPluginBehaviour).then(response => {
                ...
                if (response.offset === response.total) {
                    this.redirectToPage(`${Shuwei.Context.api.basePath}/shuwei-installer.phar.php`);
                } else {
                ...
                if (context.code === 'FRAMEWORK__PLUGIN_HAS_DEPENDANTS') {
                    ...
                    this.createNotificationError({
                        message: this.$tc('sw-settings-shuwei-updates.notifications.deactivationFailed'),
                    });
                }
            });
        },
