...
import 'src/module/sw-settings-shuwei-updates/page/sw-settings-shuwei-updates-wizard';
import 'src/app/component/structure/sw-page';
import 'src/module/sw-settings-shuwei-updates/view/sw-settings-shuwei-updates-requirements';
...
describe('module/sw-settings-shuwei-updates/page/sw-settings-shuwei-updates-wizard', () => {
    let wrapper;
    const localVue = createLocalVue();

    beforeEach(async () => {
        Shuwei.Application.view.deleteReactive = () => {};
        wrapper = shallowMount(await Shuwei.Component.build('sw-settings-shuwei-updates-wizard'), {
            localVue,
            provide: {
                updateService: {
                    ...
                    checkRequirements: () => Promise.resolve([
                        ...
                        {
                            name: 'validShuweiLicense',
                            result: false,
                            message: 'validShuweiLicense',
                            vars: [],
                            extensions: [],
                        },
                    ]),
            ...
            stubs: {
                'sw-page': await Shuwei.Component.build('sw-page'),
                'sw-search-bar': {
                    template: '<div></div>',
                },
                ...
                'sw-settings-shuwei-updates-requirements':
                    await Shuwei.Component.build('sw-settings-shuwei-updates-requirements'),
                'sw-data-grid': await Shuwei.Component.build('sw-data-grid'),
                'sw-card-view': await Shuwei.Component.build('sw-card-view'),
                'sw-card': await Shuwei.Component.build('sw-card'),
                'sw-ignore-class': true,
                'sw-settings-shuwei-updates-info': {
                    template: '<div></div>',
                },
                'sw-settings-shuwei-updates-plugins': {
                    template: '<div></div>',
                },
                'sw-loader': {
                    template: '<div></div>',
                ...
                'sw-color-badge': await Shuwei.Component.build('sw-color-badge'),
                'sw-app-actions': true,
    ...
    it('deactivate plugins success', async () => {
        ...
        expect(redirectSpy).toHaveBeenCalledWith(`${Shuwei.Context.api.basePath}/shuwei-installer.phar.php`);
    });
    ...
    it('click on update button', async () => {
        ...
        await wrapper.get('.sw-settings-shuwei-updates-wizard__start-update').trigger('click');
        await flushPromises();

        expect(wrapper.vm.updateModalShown).toBe(true);

        expect(wrapper.find('.sw-settings-shuwei-updates-check__start-update').exists()).toBe(true);

        await wrapper.get('.sw-settings-shuwei-updates-check__start-update-backup-checkbox input').setChecked(true);

        await wrapper.get('.sw-settings-shuwei-updates-check__start-update-button').trigger('click');
