...
Shuwei.Component.register('sw-extension-store-landing-page', swExtensionStoreLandingPage);

async function createWrapper() {
    const localVue = createLocalVue();
    localVue.filter('asset', v => v);

    return shallowMount(await Shuwei.Component.build('sw-extension-store-landing-page'), {
        localVue,
        stubs: {
            'sw-meteor-page': {
...
describe('src/module/sw-extension/page/sw-extension-store-landing-page', () => {
    ...
    beforeAll(() => {
        ...
        Shuwei.Utils.debug.error = jest.fn();
    });

    beforeEach(async () => {
        successfulActivation = true;
        window.location.reload.mockClear();
        Shuwei.Utils.debug.error.mockClear();
