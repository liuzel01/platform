...
export default {
    ...
    methods: {
        async changeExtensionStatus() {
            ...
            if (!this.license
                || this.license.variant !== this.shuweiExtensionService.EXTENSION_VARIANT_TYPES.RENT) {
        ...
        async activateExtension() {
            try {
                ...
                await this.shuweiExtensionService.activateExtension(
                    this.extension.name,
                    this.extension.type,
                );
        ...
        async deactivateExtension() {
            try {
                ...
                await this.shuweiExtensionService.deactivateExtension(
                    this.extension.name,
                    this.extension.type,
                );
                this.extension.active = false;
        ...
        async installExtension() {
            ...
            try {
                ...
                await this.shuweiExtensionService.installExtension(
                    this.extension.name,
                    this.extension.type,
                );
        ...
        async cancelAndRemoveExtension() {
            try {
                ...
                // by e.g. the shuwei account and the extension already has an expiration date
                if (!this.extension.storeLicense.expirationDate) {
                    await this.shuweiExtensionService.cancelLicense(this.extension.storeLicense.id);
                }

                await this.shuweiExtensionService.removeExtension(
                    this.extension.name,
                    this.extension.type,
                );
