...
import ShuweiService from 'src/module/sw-extension/service/shuwei-extension.service';

const shuweiService = new ShuweiService({}, {}, {}, {});
shuweiService.updateExtensionData = jest.fn();
const routerReplaceMock = jest.fn();

Shuwei.Component.register('sw-extension-my-extensions-listing', swExtensionMyExtensionsListing);
Shuwei.Component.register('sw-extension-my-extensions-listing-controls', swExtensionMyExtensionsListingControls);

async function createWrapper() {
    const localVue = createLocalVue();
    localVue.filter('asset', key => key);

    return shallowMount(await Shuwei.Component.build('sw-extension-my-extensions-listing'), {
        localVue,
        mocks: {
            $route: {
        ...
        stubs: {
            ...
            'sw-meteor-card': true,
            'sw-pagination': await Shuwei.Component.build('sw-pagination'),
            'sw-icon': true,
            'sw-field': true,
            // eslint-disable-next-line max-len
            'sw-extension-my-extensions-listing-controls': await Shuwei.Component.build('sw-extension-my-extensions-listing-controls'),
            ...
            'sw-select-field': await Shuwei.Component.build('sw-select-field'),
            'sw-block-field': await Shuwei.Component.build('sw-block-field'),
            'sw-alert': await Shuwei.Component.build('sw-alert'),
        },
        provide: {
            repositoryFactory: {
                create: () => {
            ...
            shuweiExtensionService: shuweiService,
        },
...
describe('src/module/sw-extension/page/sw-extension-my-extensions-listing', () => {
    ...
    beforeAll(() => {
        if (Shuwei.State.get('shuweiExtensions')) {
        ...
        Shuwei.State.registerModule('context', {
            namespaced: true,
            state: {
                app: {
                    config: {
    ...
    beforeEach(async () => {
        Shuwei.State.commit('shuweiExtensions/setExtensions', [
            {
                name: 'Test',
                installedAt: null,
            },
    ...
    it('updateList should call update extensions', async () => {
        ...
        expect(shuweiService.updateExtensionData).toHaveBeenCalled();
    });
    ...
    it('should update the route with the new values from pagination', async () => {
        ...
        Shuwei.State.commit('shuweiExtensions/setExtensions', extensions);
    ...
    it('should filter the extensions by their active state', async () => {
        ...
        Shuwei.State.commit('shuweiExtensions/setExtensions', [...activeExtensions, ...inactiveExtensions]);
    ...
    it('should show a warning if the APP_URL is not setup correctly', async () => {
        Shuwei.State.get('context').app.config.settings.appUrlReachable = false;
