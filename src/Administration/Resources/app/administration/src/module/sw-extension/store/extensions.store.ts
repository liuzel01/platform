...
interface ShuweiExtensionsState {
    search: {
        page: number,
        limit: number,
        rating: $TSFixMe,
...
const shuweiExtensionsStore: Module<ShuweiExtensionsState, VuexRootState> = {
    namespaced: true,
    ...
    mutations: {
        setSearchValue<K extends keyof ShuweiExtensionsState['search']>(
            state: ShuweiExtensionsState,
            { key, value }: SearchValue<ShuweiExtensionsState['search'], K>,
        ) {
            state.search.page = 1;
 ...
 * @private
 */
export default shuweiExtensionsStore;

/**
 * @private
 */
export type { ShuweiExtensionsState };
