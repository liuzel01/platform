...
Shuwei.Component.extend('sw-self-maintained-extension-card', 'sw-extension-card-base', swSelfMaintainedExtensionCard);

async function createWrapper() {
    return shallowMount(await Shuwei.Component.build('sw-self-maintained-extension-card'), {
        propsData: {
            extension: {
                name: 'Test',
                type: 'app',
        ...
        provide: {
            ...
            shuweiExtensionService: new ShuweiService({}, {}, {}, {}),
            cacheApiService: {
                clear() {
...
describe('src/module/sw-extension/component/sw-self-maintained-extension-card', () => {
    ...
    it('activateExtension should install and reload the page', async () => {
        ...
        wrapper.vm.shuweiExtensionService.activateExtension = jest.fn(() => Promise.resolve());

        wrapper.vm.clearCacheAndReloadPage = jest.fn(() => Promise.resolve());

        await wrapper.vm.activateExtension();

        expect(wrapper.vm.shuweiExtensionService.activateExtension).toHaveBeenCalled();
    ...
    it('deactivateExtension should install and reload the page', async () => {
        wrapper.vm.shuweiExtensionService.deactivateExtension = jest.fn(() => Promise.resolve());

        wrapper.vm.clearCacheAndReloadPage = jest.fn(() => Promise.resolve());

        await wrapper.vm.deactivateExtension();

        expect(wrapper.vm.shuweiExtensionService.deactivateExtension).toHaveBeenCalled();
        expect(wrapper.vm.isLoading).toBe(false);
    });
