    ...
    methods: {
        ...
        handleUpload(files) {
            ...
            return this.extensionStoreActionService.upload(formData).then(() => {
                Shuwei.Service('shuweiExtensionService').updateExtensionData().then(() => {
        ...
        getUserConfig() {
            return this.userConfigRepository.search(this.userConfigCriteria, Shuwei.Context.api).then(response => {
                if (response.length) {
                    this.pluginUploadUserConfig = response.first();
                } else {
                    this.pluginUploadUserConfig = this.userConfigRepository.create(Shuwei.Context.api);
        ...
        saveConfig(value) {
            ...
            this.userConfigRepository.save(this.pluginUploadUserConfig, Shuwei.Context.api).then(() => {
                this.getUserConfig();
            });
        },
    },
