...
import type { ShuweiDiscountCampaignService } from 'src/app/service/discount-campaign.service';
import type {
 ...
 * @private
 */
export default class ShuweiExtensionService {
    ...
    constructor(
        ...
        private readonly discountCampaignService: ShuweiDiscountCampaignService,
    ...
    public async updateExtensionData(): Promise<void> {
        Shuwei.State.commit('shuweiExtensions/loadMyExtensions');
        ...
        try {
            ...
            Shuwei.State.commit('shuweiExtensions/myExtensions', myExtensions);

            // await this.updateModules();
        } finally {
            Shuwei.State.commit('shuweiExtensions/setLoading', false);
        }
    }
    ...
    public async checkLogin(): Promise<void> {
        try {
            ...
            Shuwei.State.commit('shuweiExtensions/setUserInfo', userInfo);
        } catch {
            Shuwei.State.commit('shuweiExtensions/setUserInfo', null);
        }
    }
    ...
    public async getOpenLink(extension: Extension): Promise<null|RawLocation> {
        ...
        const entryRoutes = Shuwei.State.get('extensionEntryRoutes').routes;
    ...
    private getAppFromStore(extensionName: string) {
        return Shuwei.State.get('shuweiApps').apps.find((innerApp) => {
