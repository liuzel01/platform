...
export default {
    ...
    inject: ['shuweiExtensionService', 'extensionStoreActionService', 'cacheApiService'],
    ...
    computed: {
        dateFilter() {
            return Shuwei.Filter.getByName('date');
        },
        ...
        extensionMainModule() {
            return Shuwei.State.get('extensionMainModules').mainModules
    ...
    methods: {
        async createdComponent() {
            this.openLink = await this.shuweiExtensionService.getOpenLink(this.extension);
        },
        ...
        async closeModalAndUninstallExtension(removeData) {
            ...
            try {
                await this.shuweiExtensionService.uninstallExtension(
                    this.extension.name,
                    this.extension.type,
                    removeData,
                );
        ...
        async updateExtension(allowNewPermissions = false) {
            ...
            try {
                ...
                if (this.extension.installedAt) {
                    await this.shuweiExtensionService.updateExtension(
                        this.extension.name,
                        this.extension.type,
                        allowNewPermissions,
                    );
