...
Shuwei.Component.register('sw-generic-cms-page-assignment', swGenericCmsPageAssignment);
...
async function createWrapper() {
    return shallowMount(await Shuwei.Component.build('sw-generic-cms-page-assignment'), {
        stubs: {
            'sw-card': true,
            'sw-cms-list-item': {
        ...
        provide: {
            cmsPageTypeService: {
                getType(type) {
                    return {
                        title: `sw-cms.detail.label.pageType.${Shuwei.Utils.string.camelCase(type)}`,
                    };
                },
            },
            repositoryFactory: {
...
describe('module/sw-custom-entity/component/sw-generic-cms-page-assignment', () => {
    beforeEach(() => {
        if (Shuwei.State.get('cmsPageState')) {
            Shuwei.State.unregisterModule('cmsPageState');
        }
        Shuwei.State.registerModule('cmsPageState', {
            namespaced: true,
            state: {
                currentPage: null,
            },
    ...
    it('should emit slotOverrides when the cmsPage is changed', async () => {
        global.Shuwei.Data.ChangesetGenerator = class ChangesetGeneratorMock {
            generate() {
                return { changes: {
                    sections: [{
                        blocks: [{
