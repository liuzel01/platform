...
Shuwei.Component.register('sw-users-permissions-user-detail', swUsersPermissionsUserDetail);
...
async function createWrapper(privileges = []) {
    ...
    wrapper = shallowMount(await Shuwei.Component.build('sw-users-permissions-user-detail'), {
        localVue,
        provide: {
            acl: {
                can: (identifier) => {
...
<div>
    ...
    `,
            ...
            'sw-password-field': await Shuwei.Component.build('sw-text-field'),
...
describe('modules/sw-users-permissions/page/sw-users-permissions-user-detail', () => {
    beforeAll(() => {
        Shuwei.Service().register('timezoneService', () => {
            return new TimezoneService();
        });
    });

    beforeEach(async () => {
        Shuwei.State.get('session').languageId = '123456789';
        wrapper = await createWrapper();
    });

    afterEach(async () => {
        await wrapper.destroy();
        Shuwei.State.get('session').languageId = '';
    });
