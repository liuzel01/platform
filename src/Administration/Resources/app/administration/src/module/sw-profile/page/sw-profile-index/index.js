...
const { Component, Mixin, State } = Shuwei;
        ...
        loadLanguages() {
            ...
            return this.languageRepository.search(languageCriteria).then((result) => {
                ...
                result.forEach((lang) => {
                    ...
                    if (lang.locale.code === Shuwei.Context.app.fallbackLocale) {
        ...
        saveUser(context) {
            if (!this.acl.can('user:editor')) {
                ...
                this.userService.updateUser(changes.changeset[0].changes).then(async () => {
                    ...
                    Shuwei.Service('localeHelper').setLocaleWithId(this.user.localeId);
                }).catch((error) => {
                    State.dispatch('error/addApiError', {
                        expression: `user.${this.user?.id}.password`,
                        error: new Shuwei.Classes.ShuweiError(error.response.data.errors[0]),
                    });
                    this.createNotificationError({
