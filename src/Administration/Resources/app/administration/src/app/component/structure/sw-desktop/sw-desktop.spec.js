...
async function createWrapper() {
    ...
    return shallowMount(await Shuwei.Component.build('sw-desktop'), {
        localVue,
        router,
        stubs: {
            'sw-admin-menu': true,
...
describe('src/app/component/structure/sw-desktop', () => {
    beforeAll(() => {
        Shuwei.State.get('context').app.config.settings = {
    ...
    it('should call not urlDiffService when appUrlReachable is false', async () => {
        Shuwei.State.get('context').app.config.settings.appsRequireAppUrl = false;
