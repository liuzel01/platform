...
async function createWrapper() {
    return shallowMount(await Shuwei.Component.build('sw-iframe-renderer'), {
        stubs: {
...
describe('src/app/component/extension-api/sw-iframe-renderer', () => {
    beforeEach(async () => {
        ...
        Object.keys(Shuwei.State.get('extensions')).forEach((key) => {
            Vue.delete(Shuwei.State.get('extensions'), key);
        });

        // Clear sdkLocation store
        Object.keys(Shuwei.State.get('sdkLocation').locations).forEach((key) => {
            Vue.delete(Shuwei.State.get('sdkLocation').locations, key);
        });
    });
    ...
    it('should not call signIframeSrc for plugins', async () => {
        Shuwei.State.commit('extensions/addExtension', {
            name: 'foo',
    ...
    it('should render iFrame with replacement component', async () => {
        ...
        Shuwei.State.commit('sdkLocation/addLocation', {
            locationId: 'foo',
