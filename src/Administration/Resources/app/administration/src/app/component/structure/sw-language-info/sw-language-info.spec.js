...
describe('src/app/component/structure/sw-language-info', () => {
    ...
    beforeEach(async () => {
        Shuwei.State.commit('context/setApiLanguageId', '123456789');
        Shuwei.State.commit('context/setApiSystemLanguageId', '123456789');
        Shuwei.State.commit('context/setApiLanguage', {
            id: '123',
            parentId: '456',
        });

        wrapper = shallowMount(await Shuwei.Component.build('sw-language-info'), {
            stubs: {},
            mocks: {
    ...
    it('should not render the infoText when no language is set', async () => {
        Shuwei.State.commit('context/setApiLanguage', null);
    ...
    it('should not render the infoText when user is in default language', async () => {
        ...
        Shuwei.State.commit('context/setApiLanguageId', '123');
        Shuwei.State.commit('context/setApiSystemLanguageId', '123');
    ...
    it('should render the infoText for a root language', async () => {
        Shuwei.State.commit('context/setApiSystemLanguageId', '987654312');
        Shuwei.State.commit('context/setApiLanguage', {
            id: '1a2b3c',
            parentId: null,
        });
