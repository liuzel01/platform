...
async function createWrapper(errors = {}, options = {}) {
    if (typeof Shuwei.State.get('error') !== 'undefined') {
        Shuwei.State.unregisterModule('error');
    }

    Shuwei.State.registerModule('error', {
        namespaced: true,
    ...
    Shuwei.State.getters['error/getAllApiErrors'] = () => [errors];

    return shallowMount(await Shuwei.Component.build('sw-error-summary'), {
        stubs: {
