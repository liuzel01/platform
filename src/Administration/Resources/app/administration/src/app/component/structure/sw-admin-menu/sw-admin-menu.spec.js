...
const menuService = createMenuService(Shuwei.Module);
Shuwei.Service().register('menuService', () => menuService);
...
async function createWrapper(options = {}) {
    ...
    const adminMenuComponent = await Shuwei.Component.build('sw-admin-menu');

    return shallowMount(adminMenuComponent, {
        localVue,
        router: new VueRouter({ routes: Shuwei.Module.getModuleRoutes(), route: { meta: { $module: { name: '' } } } }),
        stubs: {
            'sw-icon': true,
            'sw-version': true,
            'sw-admin-menu-item': await Shuwei.Component.build('sw-admin-menu-item'),
            'sw-loader': true,
            'sw-avatar': true,
...
describe('src/app/component/structure/sw-admin-menu', () => {
    ...
    beforeAll(() => {
        ...
        Shuwei.State.registerModule('settingsItems', {
            namespaced: true,
            state: {
                settingsGroups: {
                    shop: [],
    ...
    beforeEach(async () => {
        jest.spyOn(Shuwei.Utils.debug, 'error').mockImplementation(() => true);

        Shuwei.State.commit('setCurrentUser', null);
        Shuwei.State.get('settingsItems').settingsGroups.shop = [];
        Shuwei.State.get('settingsItems').settingsGroups.system = [];

        Shuwei.Module.getModuleRegistry().clear();
        adminModules.forEach((adminModule) => {
            Shuwei.Module.register(adminModule.name, adminModule);
        });
    ...
    it('should show the snippet for the admin title', async () => {
        Shuwei.State.commit('setCurrentUser', {
            admin: true,
    ...
    it('should not render 4.level or higher menu item and throw error', async () => {
        ...
        expect(Shuwei.Utils.debug.error.mock.calls[0][0]).toBeInstanceOf(Error);
        expect(Shuwei.Utils.debug.error.mock.calls[0][0].toString()).toBe(
        ...
        expect(Shuwei.Utils.debug.error.mock.calls[1][0]).toBeInstanceOf(Error);
        expect(Shuwei.Utils.debug.error.mock.calls[1][0].toString()).toBe(
