...
Component.register('sw-admin-menu', {
    ...
    computed: {
        currentUser() {
            return Shuwei.State.get('session').currentUser;
        },

        isExpanded() {
            return Shuwei.State.get('adminMenu').isExpanded;
        },
        ...
        currentLocale() {
            return Shuwei.State.get('session').currentLocale;
        },

        currentExpandedMenuEntries() {
            return Shuwei.State.get('adminMenu').expandedEntries;
        },

        adminModuleNavigation() {
            const adminModuleNavigationEntries = Shuwei.State.get('adminMenu').adminModuleNavigation;
            ...
            return adminModuleNavigationEntries.filter((entry) => {
                ...
                if (levelThreeParent) {
                    Shuwei.Utils.debug.error(new Error(
...
The admin menu only supports up to three levels of nesting.`,
        ...
        appModuleNavigation() {
            return Shuwei.State.getters['adminMenu/appModuleNavigation'];
        },
        ...
        mainMenuEntries() {
            const tree = new Shuwei.Helper.FlatTreeHelper((first, second) => first.position - second.position);
    ...
    methods: {
        ...
        initNavigation() {
            Shuwei.State.commit('adminMenu/setAdminModuleNavigation', this.menuService.getNavigationFromAdminModules());
        ...
        refreshApps() {
            return this.appModulesService.fetchAppModules().then((modules) => {
                return Shuwei.State.commit('shuweiApps/setApps', modules);
            });
        },

        collapseAdminMenu() {
            Shuwei.State.commit('adminMenu/collapseSidebar');
        },

        expandAdminMenu() {
            Shuwei.State.commit('adminMenu/expandSidebar');
        },
        ...
        getUser() {
            ...
            this.userService.getUser().then((response) => {
                ...
                Shuwei.State.commit('setCurrentUser', userData);
        ...
        onLogoutUser() {
            ...
            Shuwei.State.commit('adminMenu/clearExpandedMenuEntries');
            Shuwei.State.commit('removeCurrentUser');
            Shuwei.State.commit('notification/setNotifications', {});
            Shuwei.State.commit('notification/clearGrowlNotificationsForCurrentUser');
            Shuwei.State.commit('notification/clearNotificationsForCurrentUser');
            this.$router.push({
                name: 'sw.login.index',
            });
        },
        ...
        onMenuItemClick(entry, eventTarget) {
            ...
            if (isEntryExpanded) {
                Shuwei.State.commit('adminMenu/collapseMenuEntry', entry);

                firstChild.classList.remove('router-link-active');
                firstChild.classList.remove('is--entry-expanded');
            } else {
                Shuwei.State.commit('adminMenu/expandMenuEntry', entry);
