...
describe('app/service/locale-helper.service.js', () => {
    ...
    beforeEach(async () => {
        localeHelperService = new LocaleHelperService({
            Shuwei: { Context: { api: {} }, State: { dispatch: () => Promise.resolve() } },
    ...
    it('setLocaleWithCode should dispatch the admin locale', async () => {
        localeHelperService._Shuwei.State.dispatch = jest.fn();

        await localeHelperService.setLocaleWithCode('testCode');

        expect(localeHelperService._Shuwei.State.dispatch).toHaveBeenCalledWith('setAdminLocale', 'testCode');
    });
});
