...
describe('src/app/service/entity-validation.service.js', () => {
    beforeAll(() => {
        Object.entries(entitySchemaMock).forEach(([entityName, definitionData]) => {
            Shuwei.EntityDefinition.add(entityName, new EntityDefinition(definitionData));
        });
    });

    it('should create a required shuwei error with the right error code and source pointer', () => {
    ...
    it('should validate a product and report callback errors', () => {
        ...
        expect(customValidator.mock.calls[0][2]).toBe(Shuwei.EntityDefinition.get(testEntity.getEntityName())); // entity definition
