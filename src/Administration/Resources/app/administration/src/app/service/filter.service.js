...
const { types } = Shuwei.Utils;
const { cloneDeep } = Shuwei.Utils.object;
...
export default class FilterService {
    ...
    getStoredFilters(storeKey) {
        ...
        return this._userConfigRepository.search(criteria, Shuwei.Context.api).then(response => {
            if (response.length) {
                this._filterEntity = response.first();
            } else {
                const currentUser = Shuwei.State.get('session').currentUser;

                this._filterEntity = this._userConfigRepository.create(Shuwei.Context.api);
    ...
    saveFilters(storeKey, filters) {
        ...
        this._pushFiltersToUrl();
        this._userConfigRepository.save(this._filterEntity, Shuwei.Context.api).then(() => {
    ...
    async _pushFiltersToUrl() {
        ...
        if (!Shuwei.Utils.types.isEmpty(routeParams)) {
