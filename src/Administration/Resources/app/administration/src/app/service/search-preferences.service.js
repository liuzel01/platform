...
export default function SearchPreferencesService({ userConfigRepository: _userConfigRepository }) {
    ...
    */
    function getDefaultSearchPreferences() {
        ...
        Shuwei.Module.getModuleRegistry().forEach(({ manifest }) => {
            if (
                manifest.entity &&
                Shuwei.Service('acl').can(`${manifest.entity}.editor`) &&
    ...
    */
    function getUserSearchPreferences() {
        return new Promise((resolve) => {
            Shuwei.Service('userConfigService').search([KEY_USER_SEARCH_PREFERENCE]).then((response) => {
    ...
    */
    function processSearchPreferencesFields(tempSearchPreferencesFields) {
        ...
        tempSearchPreferencesFields.forEach((field) => {
            field.group.forEach((group) => {
                const searchPreferencesField = Shuwei.Utils.object.set({}, group.fieldName, {
                    _searchable: field._searchable,
                    _score: field._score,
                });
                searchPreferencesFields = Shuwei.Utils.object.deepMergeObject(
    ...
    function _getUserConfigCriteria() {
        const criteria = new Shuwei.Data.Criteria();

        criteria.addFilter(Shuwei.Data.Criteria.equals('key', KEY_USER_SEARCH_PREFERENCE));
        criteria.addFilter(Shuwei.Data.Criteria.equals('userId', _getCurrentUser()?.id));
