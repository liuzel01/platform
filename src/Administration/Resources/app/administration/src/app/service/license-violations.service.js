...
export default function createLicenseViolationsService(storeService) {
    ...
    async function forceDeletePlugin(extension) {
        const shuweiExtensionService = Shuwei.Service('shuweiExtensionService');
        const cacheService = Shuwei.Service('cacheApiService');
        ...
        try {
            ...
            if (isActive) {
                await shuweiExtensionService.deactivateExtension(extension.name, extension.type);
                await cacheService.clear();
            }

            if (isInstalled) {
                await shuweiExtensionService.uninstallExtension(extension.name, extension.type);
            }

            await shuweiExtensionService.removeExtension(extension.name, extension.type);
