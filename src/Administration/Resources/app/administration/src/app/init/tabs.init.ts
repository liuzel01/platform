...
export default function initializeTabs(): void {
    Shuwei.ExtensionAPI.handle('uiTabsAddTabItem', (componentConfig) => {
        Shuwei.State.commit('tabs/addTabItem', componentConfig);
...
function createRouteForTabItem(to: Route, router: Router, next: () => void): boolean {
    ...
    Object.values(Shuwei.State.get('tabs').tabItems).find((tabItemConfigs) => {
    ...
    if (parentRoute && parentRoute.children) {
        ...
        if (!routeAlreadyExists) {
            router.addRoute(parentRoute.name, {
                path: dynamicPath,
                // @ts-expect-error
                component: Shuwei.Application.view.getComponent('sw-extension-component-section'),
                name: newRouteName,
                meta: {
                    // eslint-disable-next-line max-len
