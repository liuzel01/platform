...
async function createWrapper({
    ...
    return shallowMount({
        ...
        `,
        mixins: [
            Shuwei.Mixin.getByName('listing'),
        ],
        data() {
            return {
                ...defaultData,
...
describe('src/app/mixin/listing.mixin.ts', () => {
    ...
    beforeEach(async () => {
        ...
        if (originalWarn) {
            Shuwei.Utils.debug.warn = originalWarn;
        } else {
            originalWarn = Shuwei.Utils.debug.warn;
        }
    });
    ...
    it('should throw a console warning when no getList method ist defined', async () => {
        ...
        Shuwei.Utils.debug.warn = jest.fn(() => {});
        ...
        expect(Shuwei.Utils.debug.warn).toHaveBeenCalledWith(
            'Listing Mixin',
