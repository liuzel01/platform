...
export default Shuwei.Mixin.register('placeholder', defineComponent({
    methods: {
        placeholder<EntityName extends keyof EntitySchema.Entities>(
            ...
            if (Shuwei.Utils.types.isString(entity[field]) && entity[field].length > 0) {
                return entity[field];
            }

            // TODO: Refactor with NEXT-3304
            // Return the field from parent translation if set
            const parentLanguageId = Shuwei.Context.api.language ? Shuwei.Context.api.language.parentId : null;
