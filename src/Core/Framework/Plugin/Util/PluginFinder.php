...
use Shuwei\Core\Framework\Plugin\Composer\Factory;
...
use Shuwei\Core\Framework\Plugin\Struct\PluginFromFileSystemStruct;
...
class PluginFinder
{
    final public const COMPOSER_TYPE = 'shuwei-platform-plugin';
    private const SHUWEI_PLUGIN_CLASS_EXTRA_IDENTIFIER = 'shuwei-plugin-class';
    ...
    private function loadLocalPlugins(string $pluginDir, IOInterface $composerIO, ExceptionCollection $errors): array
    {
        ...
        try {
            ...
            foreach ($filesystemPlugins as $filesystemPlugin) {
                ...
                if (!$this->isShuweiPluginType($package) || !$this->isPluginComposerValid($package)) {
    ...
    private function isShuweiPluginType(CompletePackageInterface $package): bool
    {
    ...
    {
        return isset($package->getExtra()[self::SHUWEI_PLUGIN_CLASS_EXTRA_IDENTIFIER])
            && $package->getExtra()[self::SHUWEI_PLUGIN_CLASS_EXTRA_IDENTIFIER] !== ''
            && !empty($package->getExtra()['label']);
    }

    private function getPluginNameFromPackage(CompletePackageInterface $pluginPackage): string
    {
        return $pluginPackage->getExtra()[self::SHUWEI_PLUGIN_CLASS_EXTRA_IDENTIFIER];
    }
    ...
    private function enrichWithVendorPlugins(
        ...
        foreach ($composerPackages as $composerPackage) {
            if (!$this->isShuweiPluginType($composerPackage)) {
                continue;
            }
        ...
        if ($this->isShuweiPluginType($root) && $this->isPluginComposerValid($root)) {
        ...
        $errors->add(new PluginComposerJsonInvalidException(
            ...
            [
                sprintf(
                    ...
                    self::COMPOSER_TYPE,
                    self::SHUWEI_PLUGIN_CLASS_EXTRA_IDENTIFIER
                ),
            ]
        ));
    }
