...
use Shuwei\Core\Framework\Store\Services\AbstractExtensionLifecycle;
use Shuwei\Core\Framework\Update\Checkers\LicenseCheck;
use Shuwei\Core\Framework\Update\Checkers\WriteableCheck;
use Shuwei\Core\Framework\Update\Event\UpdatePostPrepareEvent;
use Shuwei\Core\Framework\Update\Event\UpdatePrePrepareEvent;
use Shuwei\Core\Framework\Update\Services\ApiClient;
...
class UpdateController extends AbstractController
{
    ...
    public function updateApiCheck(): JsonResponse
    {
        ...
        if (version_compare($this->shuweiVersion, $updates->version, '>=')) {
            return new JsonResponse();
        }
    ...
    public function deactivatePlugins(Request $request, Context $context): JsonResponse
    {
        ...
        if ($offset === 0) {
            ...
            $this->eventDispatcher->dispatch(
                new UpdatePrePrepareEvent($context, $this->shuweiVersion, $update->version)
            );
        }
        ...
        if ($result->getOffset() === $result->getTotal()) {
            ...
            $containerWithoutPlugins->get('event_dispatcher')->dispatch(
                new UpdatePostPrepareEvent($context, $this->shuweiVersion, $update->version)
            );
        }
