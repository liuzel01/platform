...
abstract class Plugin extends Bundle
{
     ...
     * If needed, return true and Shuwei will execute `composer require` during the plugin installation.
     * When the plugins gets uninstalled, Shuwei executes `composer remove`
     */
    ...
    {
        // namespace should not start with `shuwei`
        if (str_starts_with(mb_strtolower($this->getMigrationNamespace()), 'shuwei') && !str_starts_with(mb_strtolower($this->getMigrationNamespace()), 'shuwei\commercial')) {
            throw new \RuntimeException('Deleting Shuwei migrations is not allowed');
        }
