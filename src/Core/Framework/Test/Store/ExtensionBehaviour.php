...
use Shuwei\Core\Framework\Store\Services\AbstractStoreAppLifecycleService;
...
trait ExtensionBehaviour
{
    public function installApp(string $path, bool $install = true): void
    {
        ...
        $name = basename($path);
        $appDir = $this->getContainer()->getParameter('shuwei.app_dir') . '/' . $name;
    ...
    public function removeApp(string $path): void
    {
        ...
        $fs->remove($this->getContainer()->getParameter('shuwei.app_dir') . '/' . basename($path));
    }
