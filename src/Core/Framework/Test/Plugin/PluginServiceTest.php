...
class PluginServiceTest extends TestCase
{
    ...
    public function testRefreshPluginWithoutExtraLabelProperty(): void
    {
        ...
        $composerJsonException = $errors->filter(fn (ShuweiHttpException $error) => $error instanceof PluginComposerJsonInvalidException);

        static::assertNotEmpty($composerJsonException);

        $errorFound = false;
        $errorString = 'Plugin composer.json has invalid "type" (must be "shuwei-platform-plugin"), or invalid "extra/shuwei-plugin-class" value, or missing extra.label property';
    ...
    private function assertPluginMetaInformation(PluginEntity $plugin): void
    {
        ...
        static::assertSame('shuwei AG', $plugin->getAuthor());
        static::assertSame('(c) by shuwei AG', $plugin->getCopyright());
