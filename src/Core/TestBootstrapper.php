...
class TestBootstrapper
{
    ...
    public function addCallingPlugin(?string $pathToComposerJson = null): TestBootstrapper
    {
        ...
        $baseClass = $composer['extra']['shuwei-plugin-class'] ?? '';
        if ($baseClass === '') {
            throw new \RuntimeException('composer.json does not contain `extra.shuwei-plugin-class`. Path: ' . $pathToComposerJson);
        }
