...
class SetupDatabaseAdapter
{
    ...
    public function initializeShuweiDb(Connection $connection, ?string $database = null): bool
    {
        if (!$hasShuweiTables = $this->hasShuweiTables($connection, $database)) {
            $connection->executeStatement($this->getBaseSchema());
        }

        return !$hasShuweiTables;
    }

    public function hasShuweiTables(Connection $connection, ?string $database = null): bool
    {
        if ($database) {
